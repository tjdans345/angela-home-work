...
//name 이 왜 줄이 생기는지 
if( !nameJ.test(nameCheck) || nameCheck.trim() === ''){
	alert('형식에 맞춰 이름을 입력하세요!');
	document.getElementById('name').focus();
	return false;
}else if(!nickJ.test(nickCheck) || nickCheck.trim() === ''){
	alert('형식에 맞춰 닉네임을 입력하세요!');
	document.getElementById('nickname').focus();
	return false;
}else if(!idJ.test(idCheck) || idCheck.trim() === ''){
	alert('형식에 맞춰 아이디를 입력하세요!');
	document.getElementById('id').focus();
	return;
}else if(!pwJ.test(pwCheck) || pwCheck.trim() === ''){
	alert('숫자, 문자로 이루어진 6~12자리의 비밀번호!');
	document.getElementById('pw').focus();
	return;
}else if(!mailJ.test(emailCheck) || emailCheck.trim() === ''){
